...
export default function BusRegistrationModal({ open, onClose, onSubmit, companies = [] }: BusRegistrationModalProps) {
  ...
  const handleSubmit = () => {
    if (companies.length > 0 && !formData.companyId) {
      alert('Select a company for this bus');
      return;
    }
    onSubmit(formData);
    onClose();
  ...
  return (
    <div className="fixed inset-0 bg-navy-950/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        {/* Header */}
        <div className="p-6 border-b bg-navy-50">
            ...
            {steps.map((step, index) => (
              <div key={step.id} className="flex items-center">
                <div className={`flex items-center justify-center w-10 h-10 rounded-full ${
                  currentStep >= step.id ? 'bg-navy-500 text-white' : 'bg-gray-200 text-gray-500'
                }`}>
                  <step.icon className="w-5 h-5" />
                </div>
                <span className={`ml-2 text-sm font-medium ${
                  currentStep >= step.id ? 'text-navy-600' : 'text-gray-500'
                }`}>
                  {step.title}
                </span>
          ...
          {currentStep === 5 && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <p className="md:col-span-2 rounded-xl bg-navy-50 px-4 py-3 text-sm text-navy-700">
                  Routes are assigned later, from Route management, using buses already registered here.
                </p>
              </div>
            </div>
          )}
